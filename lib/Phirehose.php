  ...
  * @todo Shouldn't really hard-code URL strings in this function.
  ...
  protected function connect()
  {
    ...
    do {
      ...
      $response_str = trim(fgets($this->conn, 1024));
      $this->log(PHP_EOL . 'Raw response:' . PHP_EOL . $response_str, 'info');

      $httpCode = false;
      $httpMessage = '';

      // Split the response string up
      $response_args = preg_split('/\s+/', trim($response_str), 3);
      $this->log(PHP_EOL . 'Response split:' . PHP_EOL . print_r($response_args, true), 'info');
      if (!empty($response_args)) {
          // First line is response
          list($httpVer, $httpCode, $httpMessage) = $response_args;
      }
